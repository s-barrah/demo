	...
	if ($_POST) {
		...
		if(empty($_POST["email"])){
			...
			if($user_type != '' && $user_type != null){
				...
				if($userTools->checkEmailExists($email, $user_type)){
					$errors[] = '<div class="alert alert-danger text-white">This email is already registered!</div>';
				}
			}
		...
		if($user_type == 'apprentices'){
			...
			//AS APPRENTICE ID
			$apprentice_id = mt_rand(10000, 99999);
			/*while($userTools->id_is_unique($apprentice_id)){
				$apprentice_id = mt_rand(10000, 99999);
			}
			*/
			//ENSURE INPUT IS NOT EMPTY
		...
		if($user_type == 'assessors'){
			...
			if($_POST["is_accepting_new_apprentices"] == '' || $_POST["is_accepting_new_apprentices"] == null){
				$errors[] = '<div class="text-white" style="font-size:11px;">Please select an option!</div>';
			}else{
		...
		//NO ERRORS TRY TO SAVE DATA	
			...
			$user_added = $newUser->save(true, $user_type);
		
			// check for successfull registration
			if ($user_added) {
				$response['success'] = true;
				$response['notif'] = '<div class="alert alert-success text-center" role="alert"><span class="glyphicon glyphicon-ok"></span> &nbsp; User has been registered sucessfully!</div>';
				
			} else {
				$response['success'] = false;
				$response['notif'] = '<div class="alert alert-danger text-center" role="alert"><span class="glyphicon glyphicon-info-sign"></span> &nbsp; Could not register!</div>';
